...
<div x-data="adminColegio()" @keydown.escape.window="cerrar()">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-[2fr_1.4fr_1fr_1.6fr] gap-3 sm:gap-4 px-6 py-4
          ...
          <div class="flex items-center justify-start sm:justify-end gap-2">
            @if ($modalData)
              ...
              {{-- Ver (abre modal) --}}
              <button type="button"
                      @click="ver({{ Illuminate\Support\Js::from($modalData) }})"
                      title="Ver resultado"
                      class="grid place-items-center w-9 h-9 rounded-xl bg-navy-50 text-navy-700 hover:bg-navy-700 hover:text-white transition-all">
              ...
              <a href="{{ route('resultado.share.pdf', $resultado->share_token) }}"
                 title="Descargar PDF"
                 class="grid place-items-center w-9 h-9 rounded-xl bg-slate-100 text-slate-600 hover:bg-slate-700 hover:text-white transition-all">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.2" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3"/>
                </svg>
              </a>

              {{-- WhatsApp: comparte el PDF como archivo real --}}
              <button type="button"
                      @click="enviarWA({{ \Illuminate\Support\Js::from($modalData['pdf_url']) }}, {{ \Illuminate\Support\Js::from($e->nombre_completo) }})"
                      :disabled="enviando"
                      title="Enviar PDF por WhatsApp"
                      class="grid place-items-center w-9 h-9 rounded-xl bg-emerald-50 text-emerald-700 hover:bg-[#25D366] hover:text-white transition-all disabled:opacity-50">
                <svg x-show="!enviando" class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                ...
                <svg x-show="enviando" class="w-4 h-4 animate-spin" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
  ...
  <div x-show="modal.abierto"
    ...
    <div class="bg-white rounded-3xl shadow-2xl w-full max-w-xl max-h-[88vh] overflow-y-auto pointer-events-auto"
      ...
      {{-- Footer del modal --}}
      <div class="sticky bottom-0 bg-white/95 backdrop-blur border-t border-slate-100 px-6 py-4 flex gap-3 rounded-b-3xl">
        <a :href="modal.data.pdf_url"
           class="flex-1 inline-flex items-center justify-center gap-2 rounded-2xl bg-ink hover:bg-navy-700 text-white font-semibold text-sm px-5 py-3 transition-colors">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.2" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3"/>
          </svg>
          Descargar PDF
        ...
        <button type="button"
                ...
                :disabled="enviando"
                class="flex-1 inline-flex items-center justify-center gap-2 rounded-2xl bg-[#25D366] hover:bg-[#128C7E] text-white font-semibold text-sm px-5 py-3 transition-colors disabled:opacity-60">
          ...
          <span x-text="enviando ? 'Descargando PDF…' : 'Enviar por WhatsApp'"></span>
        </button>
      </div>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('adminColegio', () => ({
        ...
        // Descarga el PDF internamente y abre el selector nativo del sistema
        // para compartirlo como archivo adjunto real (no como link).
